use super::{
    ...
    BBox, Backend, DrawMode, TextChar,
};
use font::GlyphId;
...
impl TextState {
    ...
    pub fn draw_text(
        ...
        let glyphs = codepoints.map(|cid| match e.encoding {
        ...
        for (cid, gid, unicode) in glyphs {
            let is_space =
                matches!(e.encoding, TextEncoding::Cmap(_)) && unicode.as_deref() == Some(" ");
