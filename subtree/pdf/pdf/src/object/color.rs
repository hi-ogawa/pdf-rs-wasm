...
impl ColorSpace {
    fn from_primitive_depth(
        ...
        match typ {
            "Indexed" => {
                let base = Box::new(t!(ColorSpace::from_primitive_depth(
            ...
            "Separation" => {
                ...
                let alternate = Box::new(t!(ColorSpace::from_primitive_depth(
            ...
            "DeviceN" => {
                let names = t!(Object::from_primitive(
                    t!(get_index(&arr, 1)).clone(),
                    resolve
                ));
                let alt = t!(Object::from_primitive(
                    t!(get_index(&arr, 2)).clone(),
                    resolve
                ));
                let tint = t!(Function::from_primitive(
                ...
                let attr = arr
                    .get(4)
                    .map(|p| Dictionary::from_primitive(p.clone(), resolve))
                    .transpose()?;
