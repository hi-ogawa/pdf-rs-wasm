...
use crate::parser::{parse, parse_indirect_object, ParseFlags};
...
impl<B: Backend> Resolve for Storage<B> {
    fn resolve_flags(&self, r: PlainRef, flags: ParseFlags, depth: usize) -> Result<Primitive> {
        match self.changes.get(&r.id) {
            ...
            None => match t!(self.refs.get(r.id)) {
                XRef::Raw { pos, .. } => {
                    let mut lexer = Lexer::with_offset(
                        t!(self.backend.read(self.start_offset + pos..)),
                ...
                XRef::Stream { stream_id, index } => {
                    ...
                    let obj_stream = t!(self.resolve_flags(
                        PlainRef {
                            id: stream_id,
    ...
    fn get_data_or_decode(
        ...
        self.stream_cache
            .get(id, || self.decode(id, range, filters).map_err(Arc::new))
            .map_err(|e| e.into())
    }
}
...
pub fn load_storage_and_trailer_password<B: Backend>(
    ...
    let (refs, trailer) = t!(storage
        .backend
        .read_xref_table_and_trailer(storage.start_offset, storage));
    storage.refs = refs;
    ...
    if let Some(crypt) = trailer.get("Encrypt") {
        ...
        if let Some(Primitive::Reference(catalog_ref)) = trailer.get("Root") {
            let catalog = t!(t!(storage.resolve(*catalog_ref))
                .resolve(storage)?
                .into_dictionary());
...
impl<B: Backend> File<B> {
    ...
    pub fn from_data_password_with_options(
        backend: B,
        password: &[u8],
        options: ParseOptions,
    ) -> Result<Self> {
