...
fn parse_xref_section_from_stream(
    ...
    let [w0, w1, w2]: [usize; 3] = width
        .try_into()
        .map_err(|_| other!("invalid xref length array"))?;
...
pub fn parse_xref_stream_and_trailer(
    ...
    let xref_stream = t!(Stream::<XRefInfo>::from_primitive(
        Primitive::Stream(xref_stream),
        resolve
    ));
    ...
    if index.len() % 2 != 0 {
        return Err(PdfError::Other {
            msg: format!(
                "xref stream has {} elements which is not an even number",
                index.len()
            ),
        });
    }
    ...
    for (first_id, num_objects) in index.chunks_exact(2).map(|c| (c[0], c[1])) {
        let section = t!(parse_xref_section_from_stream(
            first_id,
    ...
    while lexer.peek()? != "trailer" {
        ...
        for i in 0..num_ids {
            ...
            if w1 == "trailer" {
                return Err(PdfError::Other {
                    msg: format!(
                        "xref table declares {} entries, but only {} follow.",
                        num_ids, i
                    ),
                });
            }
